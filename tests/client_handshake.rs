...
fn client_handshake() {
    ...
    client_config.action_trace = true;
    ...
    assert_eq!(
        client.action_trace,
        vec![Action::SendPacket(PacketKind::ConnectionRequest)]
    );
    assert_eq!(client.state, ClientState::SendingConnectionRequest);
    ...
    assert_eq!(
        client.action_trace,
        vec![
            Action::ReceivePacket(PacketKind::ChallengeRequest),
            Action::SendPacket(PacketKind::ChallengeResponse)
        ]
    );
    ...
    assert_eq!(
        client.action_trace,
        vec![Action::ReceivePacket(PacketKind::KeepAlive),]
    );
